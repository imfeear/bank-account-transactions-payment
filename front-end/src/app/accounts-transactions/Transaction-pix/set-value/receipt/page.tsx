...
function ReceiptContent() {
  ...
  const handleTransfer = async () => {
    const isAuthConfirmed = await confirmAuth();
  ...
  const handleDownloadReceipt = () => {
    ...
    const destinationAccount = `${accountNumber}-${agencyNumber}`;
    ...
    // Se quiser usar o logo no futuro, aqui é o lugar
    ...
    const details = [
      ...
      { label: "Conta origem:", value: originAccount },
      { label: "Conta destino:", value: destinationAccount },
      { label: "Nome destinatário:", value: userName || "Destinatário" },
      { label: "Valor:", value: `R$ ${transferAmount}` },
      {
        ...
        <Card className="w-full max-w-md shadow-lg rounded-lg">
          ...
          <CardContent>
            <div className="text-gray-600 mb-4">
              ...
              <p>
                Conta:{" "}
                <span className="text-black-600">{accountNumber}</span>
              </p>
              <p>
                Agência:{" "}
                <span className="text-black-600">{agencyNumber}</span>
              </p>
            </div>
...
export default function Receipt() {
  return (
    <Suspense fallback={<main className="p-6">Carregando recibo...</main>}>
      <ReceiptContent />
    </Suspense>
  );
}
